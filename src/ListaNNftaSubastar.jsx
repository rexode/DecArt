  ...
  async componentWillMount() {
    ...
    await this.loadWallet(this.state.uid).then((wallet) =>
      fetchNFTs(wallet)
        ...
        .then((response) => this.getOnSite(response)).then((list)=>this.setState({data:list}))
    );}
  }
          ...
          <Grid item sx={{ paddingTop: 3 }}>
            <Grid container direction="row" justifyContent="space-evenly ">
              <Grid item xs={12}>
                ...
                >
                  <Grid item>
                        <Button variant="contained"><Link to={'/LogIn'}style={{ color: '#FFF' ,textDecoration: 'none' }}>Log-In</Link></Button>
                      </Grid>
                </Grid>

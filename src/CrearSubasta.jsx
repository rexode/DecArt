...
class CrearSubasta extends Component {
  ...
  async getOnSiteSingleNft(Data) {
    ...
    if (docSnap.exists()) {
      ...
      info = { ...info, InSite: docSnap.data().InSite };
  ...
  async promiseNFt() {
    ...
    let signerAddr = await signer.getAddress().then((addr) =>
      getOwnerofNFT(this.props.params.Id).then((owner) => {
        ...
        if(addr == owner){
          const Contract = new ethers.Contract(NFTSmartContract, ERC721ABI, provider);
        ...
        }else{this.setState({BadSnackBarMessage :"Debe de conectar la cartera correcta"})
        this.BadSnackBarOpen();}
      })
    );

  }else{this.setState({BadSnackBarMessage :"Escriba la Puja minima"})
  this.BadSnackBarOpen();
  }
  ...
  async setPuja() {
    ...
    const db = getFirestore(app);
    if (this.state.Puja != 0) {
      await setDoc(doc(db, "SubastasAVerificar", this.props.params.Id), {
